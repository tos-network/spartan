...
import 'package:spartan/features/wallet/application/tos_price_provider.dart';
import 'package:spartan/features/wallet/domain/tos_price/coinpaprika/tos_ticker.dart';
import 'package:spartan/features/wallet/presentation/wallet_navigation_bar/components/qr_dialog.dart';
...
class BalanceWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final tosBalance = ref.watch(
      walletStateProvider.select((state) => state.tosBalance),
    );
    var displayedBalance =
        '${tosBalance.isNotEmpty ? tosBalance : AppResources.zeroBalance} ${getTosTicker(network)}';

    TosTicker? tosTicker;
    if (settings.showBalanceUSDT && settings.network == Network.mainnet) {
      tosTicker = ref.watch(tosPriceProvider).valueOrNull;
    }

    final usdtBalance =
        (tosTicker?.price ?? 0.0) * (double.tryParse(tosBalance) ?? 0.0);
